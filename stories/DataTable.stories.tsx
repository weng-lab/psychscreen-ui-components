...
const Template: Story<DataTableProps<any>> = (args) => <DataTable {...args} />;
...
function assayHoverInfo(assays: {
  dnase: boolean;
  h3k27ac: boolean;
  h3k4me3;
  ctcf: boolean;
...
const denseRows = [
  ...
  },
  {
    ...
    biosampleType: 'primary cell',
    queryValue:
      'dermis_microvascular_lymphatic_vessel_endothelial_cell_male_ENCDO243AAA',
    assays: {
      dnase: true,
      atac: false,
      h3k4me3: false,
...
const denseCols = [
  ...
  },
  {
    header: 'Assays',
    value: (row) =>
      Object.keys(row.assays).filter((key) => row.assays[key] === true).length,
    render: (row) => {
        ...
        <Tooltip title={assayHoverInfo(row.assays)} arrow>
          <svg height="50" width="50" viewBox="0 0 50 50">
            ...
            <circle
              ...
              strokeWidth="20"
              strokeDasharray={`${fifth * 0} ${fifth * 2} ${fifth} ${
                fifth * 2
              }`}
            />
            <circle
              ...
              strokeWidth="20"
              strokeDasharray={`${fifth * 0} ${fifth * 3} ${fifth} ${
                fifth * 1
              }`}
            />
            <circle
